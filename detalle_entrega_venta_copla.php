...
if(isset($_GET['id'])&&($_GET['id']!="")){
	...
	if($type==1){//Creando Entrega de Venta
		...
		$SQL_Seriales=Seleccionar("uvw_tbl_EntregaVentaDetalleCarrito","Count(ID_EntregaVentaDetalleCarrito) AS Cant","Usuario='".$_GET['usr']."' and CardCode='".$_GET['cardcode']."' and WhsCode='".$_GET['whscode']."' and ManSerNum='Y'");
...
<form id="from" name="form">
	...
	<table width="100%" class="table table-bordered">
		...
		<tr>
			...
			<td><?php if(($row['TreeType']!="T")||(($row['TreeType']=="T")&&($row['LineNum']!=0))){?>
				<select id="CDU_IdMetodoAplicacion<?php echo $i;?>" name="CDU_IdMetodoAplicacion[]" class="form-control m-b select2" onChange="ActualizarDatos('CDU_IdMetodoAplicacion',<?php echo $i;?>,<?php echo $row['LineNum'];?>);" <?php if(($row['LineStatus']=='C')||($type==2)){echo "disabled='disabled'";}?>>
				  <option value="">(NINGUNO)</option>
