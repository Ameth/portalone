...
if(isset($_REQUEST['dt_LS'])&&($_REQUEST['dt_LS'])==1){//Verificar que viene de una Llamada de servicio
	...
	//Sucursal cliente
	$SQL_SucursalCliente=Seleccionar('uvw_Sap_tbl_Clientes_Sucursales','*',"CodigoCliente='".base64_decode($_GET['Cardcode'])."' and TipoDireccion='S'",'TipoDireccion, NombreSucursal');
...
if(isset($_REQUEST['dt_DM'])&&($_REQUEST['dt_DM'])==1){//Verificar que viene de un documento de marketing
	...
	$SQL_DocMarketing=EjecutarSP('sp_ConsultarDocMarketing',$ParametrosDoc,0,2);
...
if($type_act==1&&$sw_error==0){//Editando la actividad
	...
	//Sucursales
	$SQL_SucursalCliente=Seleccionar('uvw_Sap_tbl_Clientes_Sucursales','*',"CodigoCliente='".$row['ID_CodigoCliente']."'",'TipoDireccion, NombreSucursal');
			   ...
			   <form action="actividad.php" method="post" class="form-horizontal" enctype="multipart/form-data" id="CrearActividad">
						...
						<div class="form-group">
							...
							<div class="col-lg-2">
								<select name="DocMarketing" class="form-control select2" id="DocMarketing" <?php if(($type_act==1)&&(!PermitirFuncion(304)||($row['IdEstadoActividad']=='Y'))){ echo "disabled='disabled'";}?>>
									...
									<?php if(($type_act==1)||($sw_error==1)||($dt_DM==1)){while($row_DocMarketing=sql_fetch_array($SQL_DocMarketing,2)){?>
				...
				<div class="ibox">
					...
					<div class="ibox-content">
						<div class="form-group">
							<div class="col-lg-11">
								<textarea name="NotasActividad" rows="10" maxlength="3000" class="form-control" id="NotasActividad" placeholder="Escriba los comentarios de los trabajos realizados..." type="text" <?php if(($type_act==1)&&(!PermitirFuncion(304)||($row['IdEstadoActividad']=='Y'))){ echo "readonly='readonly'";}?>><?php if(($type_act==1)||($sw_error==1)){echo $row['NotasActividad'];}?></textarea>
							</div>
						</div>
						<div class="form-group">
							<div class="col-lg-9">
					...
					<div class="ibox-content">
						...
						<div class="tabs-container">
							<ul class="nav nav-tabs">
								<li class="active"><a data-toggle="tab" href="#tab-1"><i class="fa fa-paperclip"></i> Anexos</a></li>
								<?php if($type_act==1){?><li><a data-toggle="tab" href="#tab-2"><i class="fa fa-map-marker"></i> Ubicación GPS</a></li><?php }?>
							</ul>
...
<script>
	 $(document).ready(function(){
		 ...
		 <?php if(PermitirFuncion(304)&&($row['IdEstadoActividad']!='Y')){?> 
...
function initMap(){
	...
	start_map(<?php echo $row['LatitudGPS'];?>,<?php echo $row['LongitudGPS'];?>);
